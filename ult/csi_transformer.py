...
def transform(db, dir):
    ...
    for date in all_month_dates:
        ...
        for stock, value in mdb.items():
            if date in value:
                mrs.append(value[date]["Monthly Return"])
        mrs = numpy.array(mrs)
        median = numpy.median(mrs)
        mean = numpy.mean(mrs)
        medians[date] = median
        means[date] = mean
    ...
    for date in all_month_dates:
        ...
        for stock in idb:
            if date in idb[stock]:
                ...
                value = numpy.array(idb[stock][date]["AMR"] + idb[stock][date]["ADR"] + [mdb[stock][date]["Monthly Return"]])
        ...
        for m, row in enumerate(all_value):
            stock = astocks[m]
            mr = mdb[stock][date]["Monthly Return"]
            median = medians[date]
            if mr > medians:
                oc = [mr,1,0]
            else:
                oc = [mr,0,1]
            output_row = numpy.concatenate( (all_value[m,:],oc), axis = 0)
            odb[date].update({stock:output_row})

    for date in odb:
        for stock in odb[date]:
            print(date,stock,odb[date][stock].shape)
            break
        # if count > 0:
